...
def readstring(format, string):
    ...
    if format != "smi":
        smiles = nci(urllib.parse.quote(string), "smiles").rstrip()
    else:
        smiles = string
    if format == "inchikey":
...
class Molecule(object):
    ...
    @property
    def formula(self): return rajweb("mf", urllib.parse.quote(self.smiles))
    @property
    def molwt(self): return float(rajweb("mw", urllib.parse.quote(self.smiles)))
    @property
    def _exchange(self):
    ...
    def calcdesc(self, descnames=[]):
        ...
        for descname in descnames:
            ...
            response = rajweb("descriptor", longname, urllib.parse.quote(self.smiles))
    ...
    def calcfp(self, fptype="std"):
        ...
        fp = rajweb("fingerprint/%s/%s" % (fptype, urllib.parse.quote(self.smiles))).rstrip()
    ...
    def write(self, format="smi", filename=None, overwrite=False):
        ...
        if format == "smi":
            ...
            try:
                output = nci(urllib.parse.quote(self.smiles), 
                             "%s" % format).rstrip().decode("utf-8").split("\n")
                #output = nci(self.smiles, "%s" % format).rstrip().split("\n")
            except urllib.error.URLError as e:
                if e.code == 404:
                    output = []
        elif format in ['inchi', 'inchikey']:
            format = "std" + format
            output = nci(urllib.parse.quote(self.smiles), "%s" % format).rstrip()
        elif format == 'iupac':
            format = format + "_name"
            try:
                output = nci(urllib.parse.quote(self.smiles), "%s" % format).rstrip()
            ...
            output = nci(urllib.parse.quote(self.smiles), "file?format=%s" % format).rstrip()
    ...
    def draw(self, show=True, filename=None):
        ...
        image display.
        """
        imagedata = nci(urllib.parse.quote(self.smiles), "image")
        if filename:
        ...
        if show:
            ...
            try:
                ...
                image = PIL.open(io.BytesIO(imagedata))
                imagedata = piltk.PhotoImage(image)
                label = tk.Label(frame, image=imagedata).pack()
                quitbutton = tk.Button(root, text="Close", command=root.destroy).pack(fill=tk.X)
                root.mainloop()
            except:
                # try damping image, works in Jupyter notebooks
                return PIL.open(io.BytesIO(imagedata))
...
class Smarts(object):
    ...
    def match(self, molecule):
        ...
        Required parameters:
           molecule
        """
        resp = rajweb("substruct", urllib.parse.quote(molecule.smiles), 
                      urllib.parse.quote(self.pat)).rstrip()
        return resp == "true"
